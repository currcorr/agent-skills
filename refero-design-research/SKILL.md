...
description: "Research-first design methodology for grounding a design in specific reference templates and real-product patterns. Use ONLY when the design work requires researching concrete references first — the user asks how top products design a screen/flow, wants a design grounded in studied references, is doing a high-stakes client-facing redesign, or explicitly mentions Refero. For everyday UI building and styling, frontend-design is the default; bring this skill in when specific templates/references must be researched and locked before implementation. Provides reference locks, decision ledgers, anti-averaging quality gates, and live Refero MCP research when available: styles for visual direction, screens for concrete UI patterns, and flows for journeys. Works without the MCP using bundled craft references."
license: MIT
metadata:
  author: referodesign
  version: "1.1"
