...
- **Hosting/residency** — API vs. self-hosted (open models like `bge`, `e5`, `nomic`, `gte`) for data-residency/compliance (Module 6.3).

**(c) Vendors.** OpenAI `text-embedding-3-{small,large}`, Cohere Embed (v3, with compression/int8 options), Voyage AI (domain variants), Google embeddings; open: BGE, E5, GTE, Nomic, Jina.
...
**(b) Mechanics.**
- **ANN indexes** trade a little recall for big speed: **HNSW** (graph-based, low-latency, memory-heavy — most common), **IVF/IVF-PQ** (clustering + quantization for memory savings), **DiskANN** (SSD-backed for huge corpora).
- **Metadata filtering** — restrict search by fields (date, source, **entitlement tags** — 3.8). *How* filtering interacts with the ANN index (pre-filter vs. post-filter) affects correctness and speed; entitlement filtering must be **pre-filter / correct**, never best-effort.
...
- **pgvector (Postgres extension)** — vectors *inside* your existing Postgres. Huge advantage: **co-locate with your relational data**, transactional consistency, familiar ops, join vectors to business tables, reuse row-level security. Best default for many enterprises up to large scale; may need tuning/partitioning at very high scale. (`pgvectorscale`/managed Postgres extend its range.)
...
- **Hybrid** — run both, then **fuse** results, commonly via **Reciprocal Rank Fusion (RRF)** (combine rankings) or weighted score blending. Captures both meaning and exact matches.
...
**(b) Mechanics.**
- **Two-stage retrieval:** fast **bi-encoder** ANN retrieval (query and docs embedded separately) → accurate **cross-encoder** reranking (query+doc scored *together*, so the model sees their interaction). Cross-encoders are too slow to run over the whole corpus but ideal on a small candidate set.
...
**(c) Tools/vendors.** Cohere Rerank, Voyage rerank, Jina reranker, BGE reranker (open), Elastic/Vertex rerankers, vendor-native rerank in RAG platforms.
...
**Multi-hop retrieval (Stabilizing).**
Some questions need chained lookups ("Which of our accounts renewing this quarter use the discontinued module?" = find renewals → find module usage → intersect). The system retrieves, reasons, retrieves again — often an **agentic RAG** loop (Module 8.6): the model decides what to fetch next. More powerful, more latency/cost, harder to evaluate.

**Contextual retrieval (Stabilizing; popularized by Anthropic, 2024).**
Before embedding each chunk, prepend a short, LLM-generated description of the chunk's context within its document (e.g., "This is from ACME's 2025 MSA, Section 7, on termination…"). This fixes the "severed context" problem (3.2) and materially improves retrieval accuracy. Often paired with BM25 (hybrid) and reranking.

**GraphRAG (Emerging → Stabilizing).**
Instead of (or alongside) vector search over text, build a **knowledge graph** (Module 4) from the corpus and retrieve over *entities and relationships*. Microsoft's **GraphRAG** and similar approaches extract entities/relations, cluster them into communities, and generate summaries — enabling **global/"connect-the-dots" questions** ("what themes span all our incident reports?") that vector RAG (which retrieves local passages) handles poorly. Also strong for multi-hop reasoning over structured relationships.
*Tradeoffs:* graph construction is expensive and can be noisy (entity-resolution errors — Module 4.5); often combined with vector RAG (**hybrid GraphRAG**) rather than replacing it. Deep integration with enterprise knowledge graphs is covered in Module 4 and its evolution in Module 8.6.

**Other patterns worth naming.** **Self-RAG / corrective RAG (CRAG)** — the model critiques retrieved context and re-retrieves or abstains if it's insufficient (reduces answering from irrelevant context). **Fusion retrieval** — multiple query variants + result fusion. **Adaptive RAG** — route easy queries to direct answer, hard ones to multi-hop.
