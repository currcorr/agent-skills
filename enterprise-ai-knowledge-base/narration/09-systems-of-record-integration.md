...
A detail caveat before we start. The agentic-CPQ specifics of both Salesforce — the Revenue Cloud and Logik.ai direction — and ServiceNow — CPQ within order-to-cash — are partly roadmap and unevenly documented publicly. The walkthrough below is the durable pattern: where the System-of-Record boundary sits, and what governance applies at each step, with vendor specifics flagged as established versus roadmap-or-thin where relevant. Verify vendor capabilities before recording.
