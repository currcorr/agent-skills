...
- **AI-specific threats.** Prompt injection (direct and **indirect** via retrieved/tool content — Modules 2.1, 2.4, 5.6), data exfiltration via tool misuse, jailbreaks, model DoS/cost attacks, insecure MCP servers, and **excessive agency** (agents with too-broad permissions). Reference: **OWASP Top 10 for LLM Applications** and **NIST AI RMF**.
...
**(c) Tools/standards.** Enterprise IAM (Okta, Entra), DLP (Microsoft Purview, others), PII detection (Presidio), secrets managers, policy engines (OPA), guardrail libraries (Module 5.6); standards: **NIST AI RMF**, **ISO/IEC 42001** (AI management systems), **OWASP LLM Top 10**, SOC 2, plus sector rules (6.6).
...
- **Cost management** — the big one:
  ...
  - **Prompt caching** — cache stable prompt prefixes (system prompt, tools, retrieved context) to cut cost/latency dramatically on repeated calls (leverages KV-cache — Module 1.5); major providers offer this.
  - **Semantic/response caching** — return cached answers for semantically-equivalent queries.
...
**(c) Tools.** Model gateways/proxies (LiteLLM, Portkey, cloud gateways, Kong AI), serving (vLLM, TGI, TensorRT-LLM), observability/eval (LangSmith, Langfuse, Arize, Datadog LLM Obs — Module 5.8), experiment/prompt management (LangSmith, PromptLayer, W&B), CI for evals (promptfoo). Native prompt caching in Anthropic/OpenAI/Google APIs.
...
- **Cross-cutting:** **EU AI Act** (risk-tiered obligations; high-risk systems face documentation, human-oversight, transparency, and logging requirements — phasing in over 2025–2027, so check current applicability); **NIST AI RMF** and **ISO/IEC 42001** as governance frameworks; **GDPR/CCPA** for personal data (automated-decision and erasure rights).
- **Financial services** — model risk management (**SR 11-7** lineage), fair-lending/**ECOA** (no discriminatory decisions; **adverse-action explainability**), **FINRA/SEC** recordkeeping and supervision of communications, AML/KYC. Heavy emphasis on explainability, audit trails, and human oversight of decisions.
- **Healthcare** — **HIPAA** (PHI protection, BAAs), FDA considerations for clinical decision tools, strict PII/PHI handling.
- **Insurance** — state regulation, unfair-discrimination rules, model governance (NAIC guidance).
