...
3. **Transactional integrity.** SoRs provide ACID guarantees, referential integrity, and workflow state machines that LLMs cannot (Module 09b).
...
- **Transactional integrity** — writes must be atomic and respect the SoR's validation rules and workflow states (all-or-nothing; no half-applied quotes). Use the SoR's transactional APIs; don't simulate transactions in the agent.
- **Idempotency** — retries (from timeouts/agent loops) must not create duplicates; use idempotency keys / dedup so "try again" doesn't double-book a quote or double-apply a discount.
- **Rollback / reversibility** — a way to undo or compensate a bad write; prefer reversible actions and design compensating transactions for high-stakes ones.
...
- **Native agent layer:** **Agentforce** — Salesforce's platform-native agents, built to act inside the CRM, with the **Atlas Reasoning Engine**, low-code agent building, and a library of actions/topics. *(Agentforce reached GA in the 2024–2025 window; versions/features evolve — verify current GA vs. roadmap.)*
- **Data backbone / grounding:** **Data Cloud** — unifies and harmonizes data (zero-copy/federation) to ground agents (09.c).
- **Governance:** the **Einstein Trust Layer** (data masking, grounding, toxicity/audit controls) plus the CRM's mature **sharing model** for entitlements — agents inherit these. This is the walled garden's biggest advantage: **governance and entitlements come largely for free** and are battle-tested.
- **CPQ direction:** legacy **Salesforce CPQ** (SteelBrick lineage) is being **succeeded by Revenue Cloud / Revenue Cloud Advanced**, and Salesforce's acquisition of **Logik.ai** signals a modern, headless, AI-friendly configuration engine direction. *(This area is actively transitioning; treat specific CPQ-agent capabilities as partly roadmap and verify — see 09.e. Public detail on exact agentic-CPQ features is uneven.)*
...
- **AI layer:** **Now Assist** (generative assistance) and **ServiceNow AI Agents / AI Agent Orchestrator** — a native **agentic** layer designed to execute and orchestrate work across ServiceNow workflows. *(Branding around "agentic"/autonomous AI has evolved rapidly across recent releases — verify current names, GA status, and scope.)*
- **Sales & Order Management / CPQ:** ServiceNow offers **Sales and Order Management** and **CPQ within an order-to-cash / telecom** context — strong where CPQ is one stage of an end-to-end fulfillment/provisioning **workflow**. *(Public technical detail on ServiceNow's agentic-CPQ specifics is comparatively thin — flag and verify.)*
...
### Stage 3 — Pricing
- **What the agent does:** computes price — list prices, volume/contract discounts, promotions, region/currency, and any negotiated terms — and explains it.
...
- **Governance (tightening sharply):** discount authority checked against the rep's role/limits (in code/SoR); discounts beyond thresholds trigger **approval workflows**; all inputs auditable. This is where **human-in-the-loop** typically enters for non-standard pricing (Modules 5.7, 09b).
- **Pitfall:** LLM computing or "estimating" price, or applying an unauthorized discount. Price is computed by the engine; discounts are authorization-gated in code.
...
- **Established:** CRM/CPQ as SoR; deterministic configuration and pricing engines; approval workflows; transactional/audited quote writes; entitlement models (Salesforce sharing, ServiceNow ACLs); Data Cloud as grounding backbone; Agentforce and ServiceNow AI Agents *exist* as native agent layers.
- **Emerging / roadmap / verify:** fully agent-driven CPQ (agent reasoning through configure→price→quote with engine calls) at both vendors; the exact division of labor between LLM and CPQ engine in shipping products; Logik.ai's role in Salesforce's agentic CPQ; ServiceNow's agentic CPQ specifics.
...
- **Platform-native vs. composable:** Salesforce = walled-garden-with-deep-integration (CRM SoR, Agentforce, Data Cloud, Trust Layer, Revenue Cloud/Logik.ai CPQ direction); ServiceNow = process/workflow-centric (Now Platform, Now Assist/AI Agents, CPQ within order-to-cash). Native for single-platform speed + inherited governance; composable for multi-SoR flexibility; most enterprises go **hybrid**. **Verify GA vs. roadmap.**
