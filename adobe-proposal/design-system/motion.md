...
Status: FINAL — adopted as the kit motion contract. Token values live in `kit/tokens.css`.
Written so a component builder can implement without judgment calls — every rule has an
exact value.
